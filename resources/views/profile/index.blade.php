        ...
        @if (! $isSocial)
            ...
            <form action="{{ route('profile.updatePassword') }}" method="POST" x-data="{ showCurrent: false, showNew: false, showConfirm: false }">
                ...
                <div class="mb-4">
                    ...
                    @error('new_password')
                        @if(str_contains($message, 'Your password must meet the following requirements'))
                            <div class="text-red-500 text-sm mt-1">
                                <p>Tu contraseña debe cumplir con los siguientes requisitos:</p>
                                <ul class="list-disc ml-5">
                                    <li>Al menos 8 caracteres</li>
                                    <li>Al menos una letra mayúscula y una letra minúscula</li>
                                    <li>Al menos un número</li>
                                    <li>Al menos un símbolo</li>
                                    <li>No encontrada en filtraciones de datos conocidas</li>
                                </ul>
                            </div>
                        @else
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    @error('new_password_confirmation')
                        @if($message == 'Your passwords do not match. Please try again.')
                            <p class="text-red-500 text-sm mt-1">Tus contraseñas no coinciden. Por favor, inténtalo de nuevo.</p>
                        @else
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @endif
                    @enderror
